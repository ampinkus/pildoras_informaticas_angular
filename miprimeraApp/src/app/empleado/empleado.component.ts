...
export class EmpleadoComponent implements OnInit {
  ...
  // apellido = 'Pinkus';
  edad: Number = 0;
  // si pongo un nobre inicial en la empresa el mismo solo aparece en el campo "Empresa del empleado" => one way binding
  // si fuese two way binding debería aparecer también en el campo "introduce el nombre de la empresa"  
  empresa = 'Google'; 
  numeroLetras: number = 0;
  ...
  // presento un alerta cuando hago click en el check box (actualmente no esta en uso)
  ...
  setUsuarioRegistrado(event: Event) {
    ...
    if ((<HTMLInputElement>event.target).value == 'si') {
      // todo esto, casting incluido, para obtener el valor de value (si o no)
      this.textoDeRegistro = 'El usuario se acaba de registrar';
    } else {
